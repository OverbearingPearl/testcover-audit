...
(ert-deftest testcover-audit-scan-test--instrument-directory-adds-load-path ()
  ...
  (let* ((dir (make-temp-file "tca-instr-loadpath" t))
         ...
         (src2-file (expand-file-name "src2.el" src2))
         (seen-load-path nil))
    (unwind-protect
        (progn
          (make-directory src2)
          (with-temp-file src1 (insert ";; dummy"))
          (with-temp-file src2-file (insert ";; dummy"))
          (cl-letf (((symbol-function 'testcover-start)
                     (lambda (_file)
                       (setq seen-load-path (append seen-load-path load-path)))))
            (let ((testcover-audit-exclude-files '("^\\."))
                  (testcover-audit-test-file-regexp nil))
              (testcover-audit-scan--instrument-directory dir)))
          (should (member (file-truename (directory-file-name dir))
                          seen-load-path))
          (should (member (file-truename (directory-file-name src2))
                          seen-load-path)))
      (delete-directory dir t))))
